(function () {
  const opportunities = (window.OrdinoraInvestmentOpportunities || []).filter((item) => item.published === true);
  ...
  function renderListing() {
    app.innerHTML = `<section class="page-hero container"><div><div class="breadcrumb"><a href="../index.html">Home</a> / <span>Investment Opportunities</span></div><p class="eyebrow">Business Facilitation</p><h1>Investment Opportunities in Brunei Darussalam</h1><p class="lead" style="margin-top:1.2rem;">Explore selected business and investment opportunities facilitated by Ordinora Business Services Sdn Bhd. These opportunities may include businesses seeking strategic investors, equity participation, joint venture partners, local partners or other commercial collaborations.</p></div></section><section class="section section--tight"><div class="container"><div class="opportunity-disclaimer" role="note">Ordinora acts as a business facilitation and introduction service. Publication of an opportunity does not constitute financial advice, investment advice, an offer of securities or a guarantee of investment returns.</div><div id="opportunity-results"></div></div></section><section class="section section--tight"><div class="container"><div class="service-cta glass opportunity-cta"><div><p class="eyebrow">For Project Owners</p><h3>Looking for Investors or Business Partners?</h3><p>Ordinora assists businesses, entrepreneurs and project owners with the promotion of selected investment opportunities, Expression of Interest campaigns, and the coordination of introductions to potential investors, strategic partners, joint-venture partners, distributors and local business partners.</p><p class="detail-note">Information submitted to Ordinora will be reviewed before publication. No investment opportunity will be publicly advertised without the project owner’s approval.</p></div><div class="hero-actions"><a href="../contact.html?service=investor-business-partner-facilitation&amp;subject=submit-investment-opportunity#contact-form" class="btn btn-primary">Submit an Investment Opportunity</a><a href="../contact.html#contact-form" class="btn btn-ghost">Contact Ordinora</a></div></div></div></section>`;
    const result = document.getElementById('opportunity-results');
    const render = () => {
      if (!opportunities.length) {
        result.innerHTML = '<div class="opportunity-empty glass"><h2>Current Investment Opportunities</h2><p>There are currently no investment opportunities available for public listing. Please check this page again for future business, partnership and investment opportunities facilitated by Ordinora Business Services Sdn Bhd.</p></div>';
        return;
      }
      ...
      result.innerHTML = matches.length ? `${featured.length ? `<section class="opportunity-group"><div class="section-head"><p class="eyebrow">Selected Projects</p><h2>Featured Opportunities</h2></div><div class="opportunity-grid">${featured.map(card).join('')}</div></section>` : ''}<section class="opportunity-group"><div class="section-head"><p class="eyebrow">Browse the register</p><h2>All Investment Opportunities</h2></div>${all.length ? `<div class="opportunity-grid">${all.map(card).join('')}</div>` : '<p class="opportunity-empty">Featured opportunities are shown above.</p>'}</section>` : '<div class="opportunity-empty glass"><h2>Current Investment Opportunities</h2><p>There are currently no investment opportunities available for public listing. Please check this page again for future business, partnership and investment opportunities facilitated by Ordinora Business Services Sdn Bhd.</p></div>';
    };
    if (opportunities.length) {
